...
const CameraBoxJamo = ({
    ...
    onPredict: (word: string, confidence: number) => void;
    isActive: boolean;
}) => {
    ...
    useEffect(() => {
        ...
        const ws = new WebSocket("ws://localhost:8000/stream/jamo");
        wsRef.current = ws;

        let interval: number;

        ws.onopen = () => {
            console.log("🟢 JAMO WebSocket 연결 성공");
        ...
        ws.onclose = () => {
            console.log("🔴 JAMO WebSocket 연결 종료됨");
